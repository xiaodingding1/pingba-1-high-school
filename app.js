...
async function loadPasswordFromCloud(){try{const r=await new AV.Query('VotePassword').equalTo('key','resetPwd').limit(1).find();if(r.length>0)globalResetPassword=r[0];else{const obj=new(AV.Object.extend('VotePassword'))();obj.set('key','resetPwd');obj.set('pwd','123456');obj.setACL(new AV.ACL({"*":{read:true,write:true}}));await obj.save();globalResetPassword=obj;}}catch(e){console.error(e);}}
...
function init(){
  ...
  floorList.forEach(f=>{
    ...
    const chk=document.createElement('input');chk.type="checkbox";chk.id=id;chk.value=f;if(assigned[f].length>=2)chk.disabled=true;
    const lbl=document.createElement('label');lbl.setAttribute('for',id);lbl.textContent=`${f} (已选 ${assigned[f].length}/2)`;if(assigned[f].length>=2)lbl.classList.add('disabled');
    wr.appendChild(chk);wr.appendChild(lbl);floorsDiv.appendChild(wr);
  });
  updatePublicBoard();
}

function getBeijingTimeString(){const d=new Date(new Date().toLocaleString("zh-CN",{timeZone:"Asia/Shanghai"}));const p=s=>String(s).padStart(2,'0');return`${String(d.getFullYear()).slice(2)}/${p(d.getMonth()+1)}/${p(d.getDate())},${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}`;}
...
async function loadVotesFromCloud(){try{const r=await new AV.Query('Vote').find();r.forEach(v=>{const n=v.get('teacher'),c=v.get('choices')||[],t=v.get('time')||'';teacherChoices[n]=c;c.forEach(f=>{if(!assigned[f])assigned[f]=[];assigned[f].push(n);});submitTime[n]=t;});init();}catch(e){console.error(e);}}

async function loadTitleFromCloud(){try{const r=await new AV.Query('VoteTitle').equalTo('key','mainTitle').limit(1).find();if(r.length>0){window.voteTitleObj=r[0];const t=r[0].get('title')||'';const el=document.getElementById('mainTitle');if(el)el.textContent=t;}}catch(e){console.error(e);}}

async function saveNewTitle(){const inp=document.getElementById('newTitleInput');if(!inp)return;const t=inp.value.trim();if(!t)return;closeModal();const el=document.getElementById('mainTitle');if(el)el.textContent=t;try{let obj=window.voteTitleObj;if(!obj){obj=new(AV.Object.extend('VoteTitle'))();obj.set('key','mainTitle');window.voteTitleObj=obj;}obj.set('title',t);obj.setACL(new AV.ACL({"*":{read:true,write:true}}));await obj.save();}catch(e){console.error(e);}}
...
async function submitChoiceHandler(){
  ...
  try{const Vote=AV.Object.extend('Vote');const r=await new AV.Query('Vote').equalTo('teacher',name).find();const obj=r.length>0?r[0]:new Vote();obj.set('teacher',name);obj.set('choices',choices);obj.set('time',submitTime[name]);obj.setACL(new AV.ACL({"*":{read:true,write:true}}));await obj.save();init();}catch(e){console.error(e);}
}
...
async function resetAll(){
  ...
  const pwd=prompt("请输入重置密码：");if(pwd===null)return;
  if(pwd!==globalResetPassword.get("pwd")){alert("密码错误！");return;}
  if(!confirm("⚠ 确定要重置所有投票吗？此操作不可恢复！"))return;
  floorList.forEach(f=>assigned[f]=[]);Object.keys(teacherChoices).forEach(k=>delete teacherChoices[k]);Object.keys(submitTime).forEach(k=>delete submitTime[k]);
  try{const r=await new AV.Query('Vote').find();for(const v of r){v.setACL(new AV.ACL({"*":{read:true,write:true}}));await v.destroy();}}catch(e){console.error(e);}
  init();
}
