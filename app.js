...
async function loadPasswordFromCloud(){
    try{
        ...
        if(r.length>0) globalResetPassword=r[0];
            ...
            const o=new (AV.Object.extend('VotePassword'))();
            o.set('key','resetPwd');
            o.set('pwd','123456');
...
function init(){
    ...
    div.innerHTML='';
    floorList.forEach(f=>{
        const id='f_'+f,wr=document.createElement('div');
...
async function saveNewTitle(){
    ...
    try{
        ...
        if(!o){
            o=new(AV.Object.extend('VoteTitle'))();
            o.set('key','mainTitle');
...
async function submitChoiceHandler(){
    ...
    try{
        const Vote=AV.Object.extend('Vote'),
              r=await new AV.Query('Vote').equalTo('teacher',name).find(),
              obj=r.length>0?r[0]:new Vote();
