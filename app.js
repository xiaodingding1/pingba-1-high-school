...
async function loadPasswordFromCloud(){
    try{
        const r = await new AV.Query('VotePassword')
            .equalTo('key','resetPwd').limit(1).find();

        if(r.length > 0){
            globalResetPassword = r[0];
        }else{
            const Obj = AV.Object.extend('VotePassword');
            const o = new Obj();
            o.set('key','resetPwd');
            o.set('pwd','123456');
            o.setACL(new AV.ACL({"*":{read:true,write:true}}));
            await o.save();
            globalResetPassword = o;
        }
    }catch(e){
        console.error(e);
    }
...
function updatePublicBoard(){
    const b = document.getElementById('publicBoard');
    if(!b) return;

    b.innerHTML = floorList.map(f =>
        assigned[f].length
            ? `${f}：${assigned[f].join('，')}`
            : `${f}：<span style="color:gray">无人选择</span>`
    ).join('<br>');
}
...
async function loadTitleFromCloud(){
    try{
        const r = await new AV.Query('VoteTitle')
            .equalTo('key','mainTitle').limit(1).find();

        if(r.length > 0){
            window.voteTitleObj = r[0];
            const el = document.getElementById('mainTitle');
            if(el) el.textContent = r[0].get('title') || '';
        }
    }catch(e){
        console.error(e);
    }
...
async function saveNewTitle(){
    ...
    try{
        let o = window.voteTitleObj;
        if(!o){
            const Obj = AV.Object.extend('VoteTitle');
            o = new Obj();
            o.set('key','mainTitle');
            window.voteTitleObj = o;
        }
...
async function submitChoiceHandler(){
    ...
    try{
        ...
        const r = await new AV.Query('Vote')
            .equalTo('teacher',name).find();
...
function closeResetModal(){const el=document.getElementById('resetPwdModal');if(el)el.style.display='none';}

async function confirmResetAll(){
    const pwd = document.getElementById('resetPwdInput')?.value.trim();
    if(!pwd || !globalResetPassword){
        alert("密码错误或未加载");return;
    }
...
window.addEventListener('DOMContentLoaded',()=>{
    Promise.all([
        ...
        [
            ['submitBtn',submitChoiceHandler],
            ['viewDetailsBtn',goToDetails],
            ['changePwdBtn',changePwdViaPrompt],
            ['openTitleBtn',openModal],
            ['closeTitleBtn',closeModal],
            ['saveTitleBtn',saveNewTitle],
            ['resetBtn',openResetModal],
            ['closeResetBtn',closeResetModal],
            ['confirmResetBtn',confirmResetAll]
        ].forEach(([id,fn])=>{
