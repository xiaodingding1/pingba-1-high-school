...
document.getElementById('submitBtn').onclick=async function(){
  ...
  if(choices.length!==2) {alert("必须选择2个楼层"); return;}
  if(teacherChoices[name]) teacherChoices[name].forEach(f=>assigned[f]=assigned[f].filter(n=>n!==name));
  for(const f of choices) if(assigned[f].length>=2) {alert(`${f} 名额已满`); return;}
...
async function resetAll(){
  if(!confirm("确定要重置所有投票吗？此操作不可撤销！")) return;
  ...
  try{
    ...
    alert("重置成功！");
  }catch(e){console.error(e); alert("重置失败！");}
  init();
}
...
// 跳转到投票详情页面
// =========================
function viewVoteDetails(){
  window.open("vote_details.html", "_blank");
}
...
window.addEventListener('DOMContentLoaded',()=>{
  ...
  detailsBtn.textContent = '查看投票详细信息';
  detailsBtn.onclick = viewVoteDetails;
  detailsBtn.style.marginLeft = '10px';
  adminPanel.appendChild(detailsBtn);
});
